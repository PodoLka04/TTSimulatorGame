...
public class L1Manager : MonoBehaviour
{
    ...
    private void InitializeGame()
    {
        ...
        tiles = new L1Tile[gridSize, gridSize];
        ...
        for (int y = 0; y < gridSize; y++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                ...
                L1Tile tile = tileObj.GetComponent<L1Tile>();
    ...
    public void OnTileClicked(Vector2Int tilePos)
    {
        ...
        foreach (var dir in directions)
        {
            ...
            if (neighborPos == emptyPos)
            {
                ...
                L1Tile clickedTile = tiles[tilePos.x, tilePos.y];
                ...
                if (CheckWinCondition())
                {
                    Debug.Log("Победа!");
                    isGameWon = true;
                    Data.Instance.Is1LevelComp = isGameWon;
    ...
    {
        L1Tile temp = tiles[pos1.x, pos1.y];
    ...
    private IEnumerator ShuffleRoutine() // Убери <T>, если он был
    {
        ...
        while (movesMade < _shuffleMoves)
        {
            ...
            {
                Vector2Int randomMove = possibleMoves[Random.Range(0, possibleMoves.Count)];
                L1Tile movingTile = tiles[randomMove.x, randomMove.y];
                RectTransform rect = movingTile.GetComponent<RectTransform>();
    ...
    private bool IsInOriginalPosition(L1Tile tile, int currentX, int currentY)
    {
